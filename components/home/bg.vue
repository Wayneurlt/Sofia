<template>
    ...
    >
      ...
      >
        ...
        <div
          ...
          class="animate-conic-gradient bg-gradient-conic absolute inset-auto right-1/2 h-56 w-60 overflow-visible from-[#FFC0CB] via-transparent to-transparent text-white opacity-50 [--conic-position:from_70deg_at_center_top]"
        >
          <div
        ...
        <div
          ...
          class="animate-conic-gradient bg-gradient-conic absolute inset-auto left-1/2 h-56 w-60 from-transparent via-transparent to-[#FFC0CB] text-white opacity-50 [--conic-position:from_290deg_at_center_top]"
        >
          <div
        ...
        <div
          class="absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-[#FFC0CB] opacity-50 blur-3xl"
        ></div>
  
        <!-- Spotlight -->
        <div
          class="animate-spotlight absolute inset-auto z-30 h-36 w-32 -translate-y-24 rounded-full bg-[#FFC0CB] blur-2xl"
        ></div>
  
        <!-- Glowing Line -->
        <div
          class="animate-glowing-line absolute inset-auto z-50 h-0.5 w-60 -translate-y-28 bg-[#FFC0CB]"
        ></div>
  ...
  const props = defineProps<LampEffectProps>();
  const durationInSeconds = computed(() => `${props.duration ?? 0.8}s`);
  const delayInSeconds = computed(() => `${props.delay ?? 0.5}s`);
  </script>
